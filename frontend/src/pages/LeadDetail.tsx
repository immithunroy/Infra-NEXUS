...
export default function LeadDetail() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    api.get<Lead>(`/leads/${id}`).then(setLead).catch((e) => flash(String(e), false)).finally(() => setLoading(false));
  }, [id]);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 pb-10">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate("/leads")} className="text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
          &larr; Back to Leads
        </button>
      </div>
      ...
      <div className="card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-700">
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">{lead.customer_name}</h1>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700/60">
          ...
          {lead.converted_to_subscriber && (
            <Row label="Converted To">
              <span className="text-emerald-600">{lead.converted_to_subscriber}</span>
            </Row>
          )}
