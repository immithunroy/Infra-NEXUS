...
for j in data:
    print(f'{j[\"name\"]:20s} status={j[\"status\"]:10s} last_run={j[\"last_run\"] or \"Never\":s} error={j[\"error\"][:50]}')
"

echo ""
echo "=== DB State ==="
docker exec infra-nexus-db psql -U olt -d infra_nexus -c "SELECT * FROM scheduler_job_state ORDER BY job_id;" 2>/dev/null
